<?php
  ...
  if(isset($_GET[' article'])){
   ...
   $article = ArticleManager::findOneById($_GET['article']);
   if($article != false){
      $article = $article[0];   

  if(isset($_POST['articl']) && $_POST['articl']== 'articl' ){
    ...
    if(count($errors)==0){
      ...
      // j'essaye de créer un objet de ArticleManager
        ...
        if($art->update($article) > 0){
            $message="Article bien Modifier!";
        }
        else{
...
<form method="post" action="">
                <div class="mb-3">
                    ...
                    <input  type="titre" name="titre" class="form-control" value="<?= $article->getTitre(); ?> >
                    
                </div>
                <div class="mb-3">
                    <label class="form-label">Contenu de l'artice</label>
                    <input  type="text" name="contenu" class="form-control" value="<?= $article->getContenu(); ?>>
                </div>
                ...
                <div class="mb-3">
		        	...
		        	<?php
				         ...
				         foreach ($categories as $categorie) {          
                            ?>
                            <option value="<?= $categorie->id; ?>" <?php if($categorie->id == $article->getCategorieId()){ echo 'checked'; } ?>><?= $categorie->nom; ?></option>
                            <?php                            }
                    ?>
		            </select>
                </div>

                <button type="submit" name="articl" value="articl" class="btn btn-primary"> Mettre à jour </button>
	...
	echo "<p>Article introuvable jkj</p>";
}
?>
 </div>
 </div>
